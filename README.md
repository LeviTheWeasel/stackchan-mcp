...
| `set_avatar(face)` | Switch avatar expression (`idle` / `happy` / `thinking` / `sad` / `surprised` / `embarrassed` / `sleep` / `yawn` / `doze`), or `off` to hide the avatar and disable blink so the underlying WiFi config / OTA / settings screens are visible. Any other face brings the avatar back and restores blink. | ✅ |
...
| `agents_converse_start(agent_id?)` | Start a hands-free, full-duplex conversation between the device and an [ElevenLabs Agents](https://elevenlabs.io/docs/eleven-agents) agent. Where `listen()` captures a fixed window and leaves end-of-turn detection to the caller, the agent does VAD-based turn detection, interruption handling, ASR and TTS itself: the mic stays open for the whole session, speech streams up as it is spoken, and replies stream back to the speaker as they are generated. The agent's own LLM produces the replies, so the gateway neither sees nor controls what is said. Listening uses the speech `voice` profile — its echo cancellation is what stops the agent from hearing its own reply and interrupting itself. Holds the microphone until stopped, so `listen()` and beat mode are mutually exclusive with it. Needs `ELEVENLABS_API_KEY` (or `STACKCHAN_ELEVENLABS_KEY`), an agent configured for `pcm_16000` audio in both directions, and the `[stt]` extra for Opus decoding. | ✅ |
| `agents_converse_stop()` | End the active conversation: close the agent socket, release the microphone, and send `listen.stop` best-effort. Returns the final session counters. | ✅ |
| `agents_converse_status()` | Poll the active conversation: running state, conversation id, uplink/playback counters, interruption count, and the last user transcript and agent reply seen. | ✅ |
